...
export default async function MakerEntityViewDesignerPage({
  ...
  const { entityLogicalName, viewLogicalName } = await params;
  ...
  const viewResponse = await apiServerFetch(
    `/api/entities/${entityLogicalName}/views/${viewLogicalName}`,
    cookieHeader,
  );
  if (viewResponse.status === 401) {
    redirect("/login");
  }
  if (viewResponse.status === 403) {
    return (
      <AccessDeniedCard
        section="Maker Center"
        title="View Designer"
  ...
  if (viewResponse.status === 404) {
    redirect(`/maker/entities/${encodeURIComponent(entityLogicalName)}/views`);
  }
  if (!viewResponse.ok) {
    throw new Error("Failed to load view.");
  }
  const view = (await viewResponse.json()) as ViewResponse;
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="space-y-1">
            <CardTitle className="font-serif text-2xl">{view.display_name}</CardTitle>
            <CardDescription>
              <span className="font-mono">{entityLogicalName}</span> ·{" "}
              <span className="font-mono">{view.logical_name}</span>
            </CardDescription>
          </div>
          <Link
