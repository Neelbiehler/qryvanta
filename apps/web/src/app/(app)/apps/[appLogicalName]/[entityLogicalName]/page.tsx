...
export default async function AppEntityPage({ params }: AppEntityPageProps) {
  ...
  const [schemaResponse, capabilitiesResponse, recordsResponse] = await Promise.all([
    apiServerFetch(
  ...
  return (
    <Card>
      <CardHeader className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.18em] text-zinc-500">Worker App</p>
          <CardTitle className="font-serif text-3xl">
            {schema.entity_display_name} · {appLogicalName}
          </CardTitle>
        </div>
        <Link href={`/apps/${appLogicalName}`} className={cn(buttonVariants({ variant: "outline" }))}>
          Back to app
        </Link>
      </CardHeader>
      <CardContent>
