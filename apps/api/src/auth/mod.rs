...
pub use session::{logout_handler, me_handler, switch_tenant_handler};
pub use step_up::step_up_handler;
...
pub const SESSION_STEP_UP_VERIFIED_AT_KEY: &str = "step_up_verified_at";
...
pub(super) const MFA_LOGIN_VERIFY_RATE_RULE: (i32, i64) = (8, 10 * 60);
pub(super) const MFA_ENROLL_CONFIRM_RATE_RULE: (i32, i64) = (8, 10 * 60);
pub(super) const MFA_MANAGEMENT_RATE_RULE: (i32, i64) = (5, 15 * 60);
pub(super) const STEP_UP_VERIFY_RATE_RULE: (i32, i64) = (8, 10 * 60);
...
pub(super) fn mfa_login_verify_rate_rule() -> RateLimitRule {
    RateLimitRule::new(
        "mfa_login_verify",
        MFA_LOGIN_VERIFY_RATE_RULE.0,
        MFA_LOGIN_VERIFY_RATE_RULE.1,
    )
}

pub(super) fn mfa_enroll_confirm_rate_rule() -> RateLimitRule {
    RateLimitRule::new(
        "mfa_enroll_confirm",
        MFA_ENROLL_CONFIRM_RATE_RULE.0,
        MFA_ENROLL_CONFIRM_RATE_RULE.1,
    )
}

pub(super) fn mfa_management_rate_rule() -> RateLimitRule {
    RateLimitRule::new(
        "mfa_management",
        MFA_MANAGEMENT_RATE_RULE.0,
        MFA_MANAGEMENT_RATE_RULE.1,
    )
}

pub(super) fn step_up_verify_rate_rule() -> RateLimitRule {
    RateLimitRule::new(
        "step_up_verify",
        STEP_UP_VERIFY_RATE_RULE.0,
        STEP_UP_VERIFY_RATE_RULE.1,
    )
}
