...
- `GET /health` returns API liveness/readiness and dependency health details (`postgres`, `redis`).
...
- `PUT /api/entities/:entity_logical_name` updates entity metadata (display name, description, plural display name, icon).
...
- `GET /api/entities/:entity_logical_name/business-rules` lists draft business rules.
- `POST /api/entities/:entity_logical_name/business-rules` creates/updates a business rule.
- `GET /api/entities/:entity_logical_name/business-rules/:business_rule_logical_name` returns one business rule definition.
- `PUT /api/entities/:entity_logical_name/business-rules/:business_rule_logical_name` updates a business rule definition.
- `DELETE /api/entities/:entity_logical_name/business-rules/:business_rule_logical_name` deletes a business rule definition.
- `POST /api/entities/:entity_logical_name/publish` publishes immutable schema version.
- `GET /api/entities/:entity_logical_name/publish-checks` runs pre-publish validation and returns publish blockers without creating a new schema version.
- `GET /api/entities/:entity_logical_name/published` returns latest published schema.
- `GET /api/publish/checks` runs workspace-level publish checks and returns aggregated blockers for entities and apps.
- `POST /api/publish/checks` validates or executes selective publish for requested entities/apps (`dry_run: true` validates only, `dry_run: false` publishes when validation passes), including dependency-aware entity relation checks for in-flight selected entities.
- `POST /api/publish/diff` returns draft-vs-published diff previews for selected entities/apps.
- `GET /api/publish/history?limit=12` returns persisted workspace publish run history for drill-down and selection replay.
- Workspace publish issue payloads may include `dependency_path` edge metadata (for example, `sales -> contact` or `contact.account_id -> account`).
- `GET /api/runtime/:entity_logical_name/records?limit=50&offset=0` lists runtime records.
- `GET /api/runtime/:entity_logical_name/business-rules` lists active business rules for runtime clients.
...
- `GET /api/apps/:app_logical_name/sitemap` returns admin sitemap configuration for one app.
- `PUT /api/apps/:app_logical_name/sitemap` saves sitemap hierarchy with validation for bound entity targets and optional `default_form` / `default_view` references.
- `GET /api/apps/:app_logical_name/publish-checks` runs app-level publish validation and returns publish blockers.
...
- `GET /api/workspace/apps/:app_logical_name/dashboards/:dashboard_logical_name` returns worker dashboard metadata for sitemap dashboard targets.
...
- `GET /api/workspace/apps/:app_logical_name/entities/:entity_logical_name/forms` lists standalone form definitions for the entity within app scope.
- `GET /api/workspace/apps/:app_logical_name/entities/:entity_logical_name/forms/:form_logical_name` returns one standalone form definition (tab/section/field layout) for the entity within app scope.
- `GET /api/workspace/apps/:app_logical_name/entities/:entity_logical_name/views` lists standalone view definitions for the entity within app scope.
- `GET /api/workspace/apps/:app_logical_name/entities/:entity_logical_name/views/:view_logical_name` returns one standalone view definition (columns/sort/filters) for the entity within app scope.
...
- `POST /api/internal/worker/jobs/claim` claims queued workflow jobs for one worker identity (`limit`, `lease_seconds`, optional `partition_count` + `partition_index`) and returns opaque per-job lease tokens for fenced completion.
- `POST /api/internal/worker/heartbeat` updates worker heartbeat and cycle counters (optional `partition_count` + `partition_index` for partition-scoped worker activity).
- `GET /api/internal/worker/jobs/stats` returns aggregate queue and active worker metrics (optional `partition_count` + `partition_index` filter).
