...
- `POST /api/runtime/:entity_logical_name/records/query` supports condition lists with logical modes (`and`/`or`).
- Filter operators include `eq`, `neq`, `gt`, `gte`, `lt`, `lte`, `contains`, and `in`.
- Query sorting supports explicit field sort order (`asc`/`desc`) for typed runtime fields.
- Legacy `filters` map payloads remain accepted and are translated to `eq` conditions for backwards compatibility.
- Filter and sort fields must exist in the published schema and pass field-type validation.
...
- `logical_mode` (`"and" | "or"`) controls how `conditions` are composed.
- `conditions` (`Array<{ field_logical_name, operator, field_value }>` ) defines typed comparisons.
- `sort` (`Array<{ field_logical_name, direction }>` ) applies stable field ordering.
- `filters` (`Record<string, unknown>`) remains an optional compatibility input for exact-match constraints.
