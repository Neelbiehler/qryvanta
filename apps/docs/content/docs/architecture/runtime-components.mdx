...
3. Worker processes claim leased jobs through authenticated internal API requests.
4. Workers execute run attempts and write run/attempt outcomes in tenant scope.
5. Completed jobs are marked terminal (`completed` or `failed`) for operator triage.
