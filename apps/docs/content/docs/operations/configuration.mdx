...
| `SMTP_HOST` | Required if `EMAIL_PROVIDER=smtp` | SMTP server host |
| `SMTP_PORT` | Required if `EMAIL_PROVIDER=smtp` | SMTP server port (`587` default) |
| `SMTP_USERNAME` | Required if `EMAIL_PROVIDER=smtp` | SMTP username |
| `SMTP_PASSWORD` | Required if `EMAIL_PROVIDER=smtp` | SMTP password |
| `SMTP_FROM_ADDRESS` | Required if `EMAIL_PROVIDER=smtp` | Sender address used for auth/invite emails |
