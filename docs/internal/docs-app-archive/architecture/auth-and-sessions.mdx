...
This keeps auth state in server-managed sessions and reduces token handling in browser code.
...
This split supports both invite-driven onboarding and open registration in local or controlled environments.
