...
Qryvanta uses PostgreSQL with SQLx.
In-memory repositories remain for test scenarios, while production paths use Postgres adapters.

Current schema covers metadata definitions, publish snapshots, runtime records, auth/session data, and tenant security tables.
